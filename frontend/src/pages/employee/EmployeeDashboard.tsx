...
export default function EmployeeDashboard() {
  ...
  const pending = goals.filter((g) => ["submitted", "under_review"].includes(g.status)).length;
  const approved = goals.filter((g) => g.status === "approved").length;
  const activeGoals = goals.filter((g) => ["approved", "submitted", "under_review", "draft"].includes(g.status));
  const recentNotifs = notifications.slice(0, 3);

  function notifIcon(type: string) {
    if (type === "success") return { icon: "thumb_up", cls: "bg-tertiary-container text-on-tertiary-container" };
    if (type === "warning") return { icon: "warning", cls: "bg-error/10 text-error" };
    if (type === "action") return { icon: "bolt", cls: "bg-primary/10 text-primary" };
    return { icon: "comment", cls: "bg-secondary-container text-on-secondary-container" };
  }
          ...
          <div className="p-md flex flex-col gap-md flex-1 overflow-y-auto">
            {recentNotifs.length === 0 ? (
              ...
              recentNotifs.map((n) => {
                ...
                return (
                  <div key={n.id} className="flex gap-md items-start">
                    ...
                    <div>
                      ...
                      <p className="text-label-md text-on-surface-variant mt-xs">{timeAgo(n.createdAt)}</p>
                    </div>
                  </div>
                );
              })
                  ...
                  <div key={goal.id} className="bg-surface-container-lowest border border-outline-variant rounded-xl p-md shadow-level-1 hover:shadow-level-2 transition-shadow flex flex-col gap-sm">
                    ...
                    <div className="mt-auto space-y-1">
                      <div className="flex justify-between text-label-sm">
                        ...
                        <span className="text-on-surface font-semibold">{goal.currentValue ?? 0}%</span>
                      </div>
